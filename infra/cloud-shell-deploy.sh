...
PROOFBID_BUILD_VERSION="${PROOFBID_BUILD_VERSION:-$(git rev-parse --verify HEAD)}"
if [[ ! "${PROOFBID_BUILD_VERSION}" =~ ^[0-9a-f]{40}$ ]]; then
  echo "PROOFBID_BUILD_VERSION must be a full Git commit SHA" >&2
  exit 2
fi
PROOFBID_IMAGE="${PROOFBID_REGION}-docker.pkg.dev/${GOOGLE_CLOUD_PROJECT}/${PROOFBID_REPOSITORY}/proofbid:${PROOFBID_BUILD_VERSION}"
...
gcloud run jobs deploy proofbid-agent \
  ...
  --max-retries 0 \
  --task-timeout 10m \
  --set-env-vars "PROOFBID_STORAGE_BACKEND=gcs,PROOFBID_TASK_BUCKET=${PROOFBID_TASK_BUCKET},PROOFBID_AGENT_MODE=google,PROOFBID_GEMINI_MODEL=gemini-3.5-flash,PROOFBID_GEMINI_AUTH=vertex_ai,GOOGLE_GENAI_USE_VERTEXAI=true,GOOGLE_CLOUD_PROJECT=${GOOGLE_CLOUD_PROJECT},GOOGLE_CLOUD_LOCATION=global,PROOFBID_ALLOWED_FIXTURES=complete_tender,PROOFBID_BUILD_VERSION=${PROOFBID_BUILD_VERSION}"
...
gcloud run deploy proofbid \
  ...
  --concurrency 20 \
  --timeout 60 \
  --set-env-vars "PROOFBID_STORAGE_BACKEND=gcs,PROOFBID_TASK_BUCKET=${PROOFBID_TASK_BUCKET},PROOFBID_CLOUD_RUN_JOB=proofbid-agent,PROOFBID_CLOUD_RUN_LOCATION=${PROOFBID_REGION},PROOFBID_PUBLIC_DEMO=1,PROOFBID_DAILY_DEMO_QUOTA=40,GOOGLE_CLOUD_PROJECT=${GOOGLE_CLOUD_PROJECT},PROOFBID_ALLOWED_FIXTURES=complete_tender,PROOFBID_BUILD_VERSION=${PROOFBID_BUILD_VERSION}"

gcloud run services describe proofbid \
  --region "${PROOFBID_REGION}" \
